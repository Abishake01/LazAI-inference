...
# Load env early so PRIVATE_KEY and other vars are available before alith imports
load_dotenv()

# Accept multiple private key env names and canonize to PRIVATE_KEY
pk = os.getenv("PRIVATE_KEY") or os.getenv("PRIVATEKEY") or os.getenv("PRIVATE_KEY_HEX")
if pk:
    os.environ["PRIVATE_KEY"] = pk

if not os.getenv("PRIVATE_KEY"):
    sys.exit(
        "Error: PRIVATE_KEY is not set. Please add PRIVATE_KEY to Build_chill_w2/.env or your environment and re-run."
    )

# Now import and instantiate the client
...
try:
    # LazAIClient may pick up PRIVATE_KEY from env; some versions accept explicit private_key
    client = LazAIClient()
except TypeError:
    client = LazAIClient(private_key=os.getenv("PRIVATE_KEY"))
